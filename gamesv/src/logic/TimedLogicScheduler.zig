...
pub fn shouldDrain(state: *State, now_ms: i64) bool {
    ...
    return isDue(state.next_timed_logic_check_ms, now_ms) or
        isDue(state.next_levelplay_timer_tick_ms, now_ms) or
        isDue(state.next_scene_cleanup_tick_ms, now_ms) or
        isDue(state.next_dirty_save_tick_ms, now_ms);
}
